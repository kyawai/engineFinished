...
void Pause::RenderInitialise(char * _shader)
{
	numb = 0;
	currImg = 0;
	std::sr1::shared_ptr<Application> app = getApp();
...
void Pause::setPosition(glm::vec4 _size)
{
	pos = _size;
}

void Pause::onDisplay()
{
...
void Pause::getInputCont(int _num)
{
	numb = _num;
}
...
void Pause::onGetHit(int _hitNum)
{
	hitNum = _hitNum;
}